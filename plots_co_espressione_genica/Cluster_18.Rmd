...
results_hdwgcna <- file.path(base_path, "results/hdwgcna/cluster_18")
hdwgcna_path <- file.path(base_path, "intermediate/hdwgcna/cluster_18" )
grafici_path <- file.path(base_path, "grafici/hdwgcna/cluster_18")
bone_multi <- readRDS(file.path(hdwgcna_path, "C18_hdwgcna_full.rds"))
DMEs <- readRDS(file.path(hdwgcna_path, "C18_DME_results.rds"))
scores_C18 <- readRDS(file.path(hdwgcna_path, "C18_module_scores.rds"))
```

```{r underscore, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}
score_cols <- colnames(scores_C18)
for (col in score_cols) {
  bone_multi[[col]] <- NA
}
common_cells <- intersect(rownames(bone_multi@meta.data), rownames(scores_C18))
bone_multi@meta.data[common_cells, score_cols] <- 
  scores_C18[common_cells, , drop = FALSE]
```
...
bone_multi_c18 <- subset(bone_multi, subset = clusters_res_0_10 == "18")

p_c18 <- DimPlot(
  bone_multi,
  cells.highlight = colnames(bone_multi)[bone_multi$clusters_res_0_10 == "18"],
  cols            = "grey80",   
  cols.highlight  = "red",      
  pt.size         = 0.1,        
  sizes.highlight = 0.5           
...
if (length(idx_tbx1) == 0) {
  ...
  if (is.numeric(tbx1_mod_raw)) {
    tbx1_mod <- paste0("C18_M", tbx1_mod_raw)
  } else {
...
bone_multi_c18$condition <- factor(bone_multi_c18$condition, levels = c("WT", "MUT"))
